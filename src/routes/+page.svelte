...
<main>
  <nav class="flex flex-row absolute bottom-0 left-0 w-full justify-center h-[150px] bg-red-400">
    {#each collection as { name, component }}
      {#if name != "About"}
        <button class="flex w-1/3 text-center self-center justify-center" on:click={() => selectComponent(component)}>
          {#if name === "Recent"}
